...
def generate_induced_subgraph_game(distribution, n_agents, sparsity=0.0, **kwargs):
    ...
    connected_edges = [(i + 1, j + 1) for i, j in nx.random_tree(n_agents, seed=123).edges] 
...
#             print(coalition)
            skipflag = False
            for agent_1 in coalition:
                if True not in [','.join(map(str,sorted([int(agent_1),int(agent_2)]))) in induced_subgraph_game for agent_2 in [x for x in coalition if x!=agent_1]]:
                    skipflag = True
...
# { 1:0 , 2: 0, 3:0, 12:-20, 13:34, /123:55}
